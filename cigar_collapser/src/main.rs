...
fn run_args_mode() -> ExitCode {
    ...
    for arg in work_args {
        match make_collapsed_cigar(&arg) {
            Ok(ok_str) => {
                println!("{}", ok_str);
            },
            Err(err_str) => {
...
fn run_stdin_mode() -> ExitCode  {
    ...
    for line_result in reader.lines() {
        ...
        match make_collapsed_cigar(&line) {
            Ok(ok_str) => {
                println!("{}", ok_str);
            },
            Err(err_str) => {
...
fn make_collapsed_cigar(str_arg: &String) -> Result<String, String> {
    let mut q_len:          u32 = 0;
...
#[cfg(test)]
mod test_make_collapsed_cigar {
    use super::{make_collapsed_cigar};
    ...
    fn test_cigar_ok_with_clips() {
        ...
        let left: String = make_collapsed_cigar(&cigar_str).unwrap();
    ...
    fn test_cigar_err_empty_operation_len() -> Result<(), ()> {
        ...
        if let Err(err_str) = make_collapsed_cigar(&cigar_str) {
